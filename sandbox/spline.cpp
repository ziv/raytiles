...
int main() {
  ...
  std::vector<Vector3> path = {
      {2468.0f, 1339.0f, 380.0f},   {1525.0f, 1054.0f, 1230.0f},    {509.0f, 963.0f, 2326.0f},      {-596.0f, 1012.0f, 2768.0f},
      {-1493.0f, 1066.0f, 3736.0f}, {-2157.0f, 998.0f, 4946.0f},    {-2736.0f, 1036.0f, 5688.0f},   {-2890.0f, 1121.0f, 6991.0f},
      {-3367.0f, 1231.0f, 7762.0f}, {-13233.0f, 4759.0f, 20554.0f}, {-15447.0f, 5597.0f, 23428.0f},
  };
  ...
  auto absolute_to_user = [&](const Vector3 abs) { return Vector3Add(abs, world_offset); };
  ...
  while (!WindowShouldClose()) {
    ...
    if (auto_pilot && !crashed) {
      ...
      camera.position = absolute_to_user(get_spline_point(path[step % path.size()], path[(step + 1) % path.size()], path[(step + 2) % path.size()],
                                                          path[(step + 3) % path.size()], flight_progress));

      // look ahead...
      camera.target = absolute_to_user(get_spline_point(path[(step + 1) % path.size()], path[(step + 2) % path.size()], path[(step + 3) % path.size()],
                                                        path[(step + 4) % path.size()], flight_progress + 0.2f));
    }
    ...
    DrawText(TextFormat("user P %d %d %d", static_cast<int>(camera.position.x), static_cast<int>(camera.position.y), static_cast<int>(camera.position.z)), 10,
             20, 20, WHITE);
    DrawText(TextFormat("offset %d %d %d", static_cast<int>(world_offset.x), static_cast<int>(world_offset.y), static_cast<int>(world_offset.z)), 10, 50, 20,
             WHITE);
