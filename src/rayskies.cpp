...
namespace raytiles::sky {
    sky_steamer::sky_steamer(const sky_config &config)
        : renderer(std::make_unique<sky_renderer>(config)) {
    }

    sky_steamer::~sky_steamer() = default;

    void sky_steamer::draw(const Vector3 player_pos) const {
        renderer->draw(player_pos);
    }

    void sky_steamer::set_horizon_color(const float r, const float g, const float b) const {
    ...
    void sky_steamer::set_zenith_color(float r, float g, float b) const {
