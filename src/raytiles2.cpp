...
namespace raytiles {
    ...
    // streamer2::streamer(const world_config &world_conf,
    ...
    // }
    //
    // streamer2::streamer(const double latitude,
    ...
    configuration::configuration(const double latitude, const double longitude) {
        ...
        const double x = (longitude + 180.0) / 360.0 * n;
        const double y = (1.0 - std::log(std::tan(lat) + 1.0 / std::cos(lat)) / PI) / 2.0 * n;
        anchor_x_tile = static_cast<int>(std::floor(x));
        anchor_z_tile = static_cast<int>(std::floor(y));

        // calculate tile size
        constexpr double equator_circumference_m = 40075016.686;
        const double tile_size = equator_circumference_m * std::cos(lat) / n;
        base_zoom_tile_size = static_cast<float>(tile_size);

        // calculate the offset
        const auto offset_x = static_cast<float>((x - anchor_x_tile) * tile_size);
        const auto offset_z = static_cast<float>((y - anchor_z_tile) * tile_size);
    ...
    streamer2::streamer2(streamer2 &&) noexcept = default;

    streamer2 &streamer2::operator=(streamer2 &&) noexcept = default;

    bool streamer2::is_loading() const {
        return tile_manager->is_loading();
    }

    float streamer2::get_loading() const {
        return tile_manager->get_loading();
    }

    Vector3 streamer2::get_initial_position(const float y) const {
        return init_position + Vector3{0.0f, y, 0.0f};
    }

    std::optional<float> streamer2::ground_height(const Vector3 position) const {
        return tile_manager->ground_height(Vector3Subtract(position, cached_world_offset_));
    }

    void streamer2::update(const Camera3D &camera, const Vector3 world_offset) {
    ...
    void streamer2::draw_debug_3d() {
        tiles_renderer::debug_3d(cached_world_offset_, tile_manager->make_debug_view(last_frustum));
    }

    void streamer2::draw_debug_labels() {
        tiles_renderer::debug(cached_camera_, cached_world_offset_, tile_manager->make_debug_view(last_frustum));
    }

    void streamer2::set_ambient_light(const Color color) const { tile_renderer->set_ambient_light(color); }
    void streamer2::set_ambient_light(const Vector4 color) const { tile_renderer->set_ambient_light(color); }
    void streamer2::set_ambient_light(const float r, const float g, const float b, const float a) const { tile_renderer->set_ambient_light(r, g, b, a); }
    void streamer2::set_fog_color(const Color color) const { tile_renderer->set_fog_color(color); }
    void streamer2::set_fog_color(const Vector4 color) const { tile_renderer->set_fog_color(color); }
    void streamer2::set_fog_color(const float r, const float g, const float b, const float a) const { tile_renderer->set_fog_color(r, g, b, a); }
    void streamer2::set_fog_start(const float distance) const { tile_renderer->set_fog_start(distance); }
    void streamer2::set_fog_end(const float distance) const { tile_renderer->set_fog_end(distance); }
    void streamer2::set_height_scale(const float scale) const { tile_renderer->set_height_scale(scale); }
    void streamer2::set_normals_scale(const float scale) const { tile_renderer->set_normals_scale(scale); }
    void streamer2::set_sun_direction(const Vector3 direction) const { tile_renderer->set_sun_direction(direction); }
    void streamer2::set_sun_scale(const float scale) const { tile_renderer->set_sun_scale(scale); }
} // namespace raytiles
