...
namespace raytiles {
    namespace {
        ...
        raii::image fetch_asset(const source_options &opts,
                                std::unordered_map<std::string, httplib::Client> &clients,
                                const asset kind, const int zoom, const int abs_x, const int abs_z) {
            const auto &cache_template = kind == asset::texture
                                             ? opts.texture_cache_path
                                             : kind == asset::heightmap
                                                   ? opts.heightmap_cache_path
                                                   : opts.normals_cache_path;
            const auto &host = kind == asset::texture
                                   ? opts.texture_host
                                   : kind == asset::heightmap
                                         ? opts.heightmap_host
                                         : opts.normals_host;
            const auto &url_template = kind == asset::texture
                                           ? opts.texture_url_path
                                           : kind == asset::heightmap
                                                 ? opts.heightmap_url_path
                                                 : opts.normals_url_path;
    ...
    tile_source::tile_source(source_options opts)
