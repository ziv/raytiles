...
inline MetersDSq distance_sq_to_tile(const Vector3& position, const tile_key& tile, const float tile_size) {
  ...
  return static_cast<double>(dx) * static_cast<double>(dx) + static_cast<double>(dz) * static_cast<double>(dz) +
...
inline MetersDSq calculate_horizon(const Vector3& position) { return horizon_ratio * horizon_ratio * std::max(position.y, 1.0f); }
...
inline bool is_tile_in_frustum(const float x, const float z, const float size, const Frustum& frustum) {
  ...
  for (const auto [normal, distance] : frustum.planes) {
    ...
    const float d = normal.x * p.x + normal.y * p.y + normal.z * p.z + distance;
