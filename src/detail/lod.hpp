...
inline void build_required(const options& o, const derived& d, const Vector3& position, const Zoom zoom, const int tx, const int tz, const MetersDSq horizon_sq,
  ...
  for (int ox = 0; ox < 2; ++ox)
    for (int oz = 0; oz < 2; ++oz) build_required(o, d, position, child_zoom, cx0 + ox, cz0 + oz, horizon_sq, out);
}
}  // namespace detail
...
inline void desired_tiles(const options& o, const Vector3& position, std::unordered_set<tile_key>& out) {
  ...
  for (int dx = -r; dx <= r; ++dx)
    for (int dz = -r; dz <= r; ++dz)
      if (dz * dz + dx * dx < allowed_radius) detail::build_required(o, d, position, o.base_zoom, current_tile_x + dx, current_tile_z + dz, horizon_sq, out);
}
}  // namespace raytiles::lod
