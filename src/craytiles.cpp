...
static_assert(RAYTILES_ZOOM_LEVELS == raytiles::zoom_levels, "RAYTILES_ZOOM_LEVELS must match raytiles::zoom_levels");
...
struct RaytilesStreamer {
  ...
  explicit RaytilesStreamer(raytiles::config conf) : impl(std::move(conf)) {}

  RaytilesStreamer(const double latitude, const double longitude, raytiles::config conf) : impl(latitude, longitude, std::move(conf)) {}
};
...
RaytilesStreamer* RaytilesStreamerCreateLatLon(const double latitude, const double longitude, const RaytilesConfig* config) {
  try {
...
void RaytilesStreamerDestroy(RaytilesStreamer* streamer) { delete streamer; }
...
bool RaytilesStreamerGroundHeight(const RaytilesStreamer* streamer, const Vector3 position, float* out_height) {
  if (!streamer) return false;
