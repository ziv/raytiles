...
namespace raytiles {
    ...
    streamer::streamer(config conf)
        : impl_(std::make_unique<impl>(conf)) {
    }

    streamer::streamer(const double latitude, const double longitude, config conf)
        : streamer(with_geo_anchor(std::move(conf), latitude, longitude)) {
    }
    ...
    bool streamer::is_loading() const {
        return impl_->store.is_loading();
    }

    float streamer::loading_progress() const {
        return impl_->store.get_loading();
    }

    Vector3 streamer::initial_position(const float altitude) const {
        return impl_->init_position + Vector3{0.0f, altitude, 0.0f};
    }
    ...
    void streamer::draw() {
        impl_->rendered = impl_->renderer.draw(impl_->cached_camera.position, impl_->store.render_items());
    }

    void streamer::draw_debug_3d() {
        terrain_renderer::debug_3d(impl_->store.render_items());
    }

    void streamer::draw_debug_labels() {
        terrain_renderer::debug(impl_->cached_camera, impl_->store.render_items());
    }
