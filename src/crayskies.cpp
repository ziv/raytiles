...
struct RaytilesSkyStreamer {
    ...
    explicit RaytilesSkyStreamer(const raytiles::sky::sky_config &config)
        : impl(config) {
    }
};
...
void RaytilesSkyStreamerSetHorizonColorRGB(RaytilesSkyStreamer *sky,
...
void RaytilesSkyStreamerSetZenithColorRGB(RaytilesSkyStreamer *sky,
                                          const float r, const float g, const float b) {
    if (!sky) return;
