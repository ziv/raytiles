...
- **Errata (found by the author running the demo, fixed post-close-out):** moving the
  loading-finished flip from `post_process` into `promote` was *not* behavior-identical as claimed
  below — `promote` runs before the first `update_desired`, so the flag flipped on frame one with
  nothing yet requested and the loading screen never showed. Fixed by guarding the flip on
  `!desired_keys.empty()` ("a desired set exists and is fully serviced"). Lesson recorded: "X only
  observed after update() returns" does not make intra-frame reordering safe when the state X reads
  is written *later in the same frame*.
