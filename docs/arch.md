...
### Synthesis above the native zoom

Terrain providers stop at `network_config::native_terrain_zoom` (Mapzen: 15) while imagery goes
deeper. Above that zoom the source *synthesizes* heightmaps instead of fetching: on a cache miss
the worker fetches the native-zoom ancestor, decodes it to float heights, upsamples only the
quadrant chain containing the target (heights are interpolated in height space — Terrarium RGB
channels can't be lerped across carry boundaries), re-encodes, caches the PNG like a fetched tile,
and serves it in the same job. The siblings along the requested tile's ancestry are then generated by a
**background derive task** — a second, lower-priority queue the workers only touch when no real
job waits, so backfill never delays a requested tile (lineage siblings only: a full subtree would
be ~21k tiles per parent at z22). Normals above the native zoom (and on any
fetch/decode failure below it) fall back to a flat default — normals never fail a tile. Nothing
above `tile_source` knows synthesis exists; derived tiles are ordinary cache entries. Details and
the decision log: [`greater-zoom-plan.md`](greater-zoom-plan.md).
