...
and serves it in the same job. The ancestor's remaining descendants are then generated by a
**background derive task** — a second, lower-priority queue the workers only touch when no real
job waits, so backfill never delays a requested tile. Normals above the native zoom (and on any
